...
def log_engine(mysql_conn, catalog_entry):
    ...
    if is_view:
        user_logger.info("Beginning sync for view %s.%s", database_name, catalog_entry.table)
    else:
        with connect_with_backoff(mysql_conn) as open_conn:
            with open_conn.cursor() as cur:
                ...
                if row:
                    user_logger.info("Beginning sync for %s table %s.%s", row[0], database_name, catalog_entry.table)
...
def binlog_stream_requires_historical(catalog_entry, state):
    ...
    gtid = singer.get_bookmark(state, catalog_entry.tap_stream_id, "gtid")
...
def get_non_binlog_streams(mysql_conn, catalog, config, state):
    ...
    for stream in selected_streams:
        ...
        if not stream_state:
            if replication_method == "LOG_BASED":
                internal_logger.info("LOG_BASED stream %s requires full historical sync", stream.tap_stream_id)
            ...
            if is_view:
                raise Exception(f"Unable to replicate stream({stream.stream}) with binlog because it is a view.")

            internal_logger.info("LOG_BASED stream %s will resume its historical sync", stream.tap_stream_id)
...
def do_sync_incremental(mysql_conn, catalog_entry, state, columns):
    user_logger.info("Stream %s is using incremental replication", catalog_entry.stream)
...
def do_sync_historical_binlog(mysql_conn, catalog_entry, state, columns, use_gtid: bool, engine: str):
    ...
    if max_pk_values and ((use_gtid and gtid) or (log_file and log_pos)):
        user_logger.info("Resuming initial full table sync for LOG_BASED stream %s", catalog_entry.tap_stream_id)
        full_table.sync_table(mysql_conn, catalog_entry, state, columns, stream_version)
    else:
        user_logger.info("Performing initial full table sync for LOG_BASED stream %s", catalog_entry.tap_stream_id)

        state = singer.write_bookmark(state, catalog_entry.tap_stream_id, "initial_binlog_complete", False)
        ...
        if full_table.pks_are_auto_incrementing(mysql_conn, catalog_entry):
            ...
            # an incrementing PK
            state = singer.write_bookmark(state, catalog_entry.tap_stream_id, "log_file", current_log_file)

            state = singer.write_bookmark(state, catalog_entry.tap_stream_id, "log_pos", current_log_pos)

            if current_gtid:
                state = singer.write_bookmark(state, catalog_entry.tap_stream_id, "gtid", current_gtid)
...
def do_sync_full_table(mysql_conn, catalog_entry, state, columns):
    user_logger.info("Stream %s is using full table replication", catalog_entry.stream)
...
def sync_non_binlog_streams(mysql_conn, non_binlog_catalog, state, use_gtid, engine):
    for catalog_entry in non_binlog_catalog.streams:
        ...
        if not columns:
            user_logger.warning("There are no columns selected for stream %s, skipping it.", catalog_entry.stream)
            continue
...
def do_sync(mysql_conn, config, catalog, state):
    ...
    sync_non_binlog_streams(mysql_conn, non_binlog_catalog, state, config["use_gtid"], config["engine"])
...
def log_server_params(mysql_conn):
    with connect_with_backoff(mysql_conn) as open_conn:
        try:
            ...
            internal_logger.warning("Encountered error checking server params. Error: (%s) %s", *exc.args)
