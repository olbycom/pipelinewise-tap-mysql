...
if TYPE_CHECKING:
    ...
    from tap_mysql.streams import MySQLLogBasedStream


class MySQLSingleLogBasedStream(SQLStream):
    """Stream class for MySQL streams."""

    connector_class = MySQLConnector
    replication_key = "_sdc_lsn"
    log_based_streams: list["MySQLLogBasedStream"] = []
    ...
    def fast_forward_to_latest_binlog(self):
        """Fast-forward the stream state to the latest binlog position."""
        internal_logger.info("Fast-forwarding stream state to the latest binlog position.")
        ...
        try:
            with self.connector._connect() as conn:
                ...
                if not binary_logs:
                    user_logger.warning("No binary logs found. Cannot fast-forward state.")
                    return
                latest_log = binary_logs[-1]
                latest_log_file = latest_log[0]
                latest_log_pos = latest_log[1]
        except Exception:
        ...
        self._write_state_message()
        user_logger.info(f"State fast-forwarded to binlog file {latest_log_file}, position {latest_log_pos}.")
