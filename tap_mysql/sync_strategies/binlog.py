    ...
    with connect_with_backoff(mysql_conn) as open_conn:
        with open_conn.cursor() as cur:
            ...
            if binlog_format != "ROW":
                raise Exception(
                    f"Unable to replicate binlog stream because binlog_format is " f"not set to 'ROW': {binlog_format}."
                )
...
def fetch_current_gtid_pos(mysql_conn: MySQLConnection, engine: str) -> str:
    ...
    with connect_with_backoff(mysql_conn) as open_conn:
        with open_conn.cursor() as cur:
            ...
            gtids = result[0]
            internal_logger.debug("Found GTID(s): %s in server %s", gtids, server)
            ...
            if gtid_to_use:
                internal_logger.info("Using GTID %s for state bookmark", gtid_to_use)
                return gtid_to_use
...
def row_to_singer_record(catalog_entry, version, db_column_map, row, time_extracted):
    ...
    return singer.RecordMessage(
        stream=catalog_entry.stream, record=row_to_persist, version=version, time_extracted=time_extracted
    )
...
def calculate_gtid_bookmark(
    ...
    if not min_gtid:
        ...
        internal_logger.info(
            "Couldn't find a gtid in state, will try to infer one from binlog coordinates if they exist .."
        )
        ...
        internal_logger.info('The inferred GTID is "%s", it will be used to resume replication', min_gtid)
    else:
        internal_logger.info(
...
def _find_gtid_by_binlog_coordinates(mysql_conn: MySQLConnection, log_file: str, log_pos: int) -> Optional[str]:
    ...
    internal_logger.debug("BINLOG_GTID_POS returned gtids: %s", gtids)
...
def update_bookmarks(state: Dict, binlog_streams_map: Dict, log_file: str, log_pos: int, gtid: Optional[str]) -> Dict:
    ...
    internal_logger.debug(
        "Updating state bookmark to binlog file and pos and GTID: %s, %d, %s", log_file, log_pos, gtid
    )
...
def handle_delete_rows_event(event, catalog_entry, state, columns, rows_saved, time_extracted):
    ...
    event_ts = datetime.datetime.utcfromtimestamp(event.timestamp).replace(tzinfo=pytz.UTC).isoformat()
...
def __get_diff_in_columns_list(
    ...
    binlog_columns_filtered = filter(
        lambda col_name, ignored_cols=ignore_columns: not bool(
            re.match(r"__dropped_col_\d+__", col_name) or col_name in ignored_cols
        ),
...
def _run_binlog_sync(
    ...
    for binlog_event in reader:
        ...
        if isinstance(binlog_event, RotateEvent):
            internal_logger.debug(
                "RotateEvent: log_file=%s, log_pos=%d", binlog_event.next_binlog, binlog_event.position
            )
            ...
            internal_logger.debug("%s: gtid=%s", binlog_event.__class__.__name__, gtid_pos)
            ...
            if not catalog_entry:
                ...
                if diff:
                    ...
                    internal_logger.info(
                        "Stream `%s`: Difference detected between event and schema: %s", tap_stream_id, diff
                    )
                    ...
                    if not should_run_discovery(diff, md_map):
                        ...
                        internal_logger.info("Stream `%s`: Running discovery ... ", tap_stream_id)

                        # run discovery for the current table only
                        new_catalog_entry = discover_catalog(
                            mysql_conn, config.get("filter_dbs"), catalog_entry.table
                        ).streams[0]

                        selected = {
                            k
                            for k, v in new_catalog_entry.schema.properties.items()
                ...
                if isinstance(binlog_event, WriteRowsEvent):
                    processed_rows_events = handle_write_rows_event(
                        binlog_event, catalog_entry, state, columns, processed_rows_events, time_extracted
                    )
    ...
    internal_logger.info("Processed %s rows", processed_rows_events)
...
def create_binlog_stream_reader(
    ...
    if config.get("server_id"):
        ...
        internal_logger.info("Using provided server_id=%s", server_id)
    else:
    ...
    if config["use_gtid"]:
        ...
        internal_logger.info("Starting logical replication from GTID '%s' on engine '%s'", gtid_pos, engine)
        ...
        internal_logger.info("Starting logical replication from binlog file ['%s', %d]", log_file, log_pos)
...
def sync_binlog_stream(
    ...
    try:
        ...
        internal_logger.info("Current Master binlog file and pos: %s %s", end_log_file, end_log_pos)
