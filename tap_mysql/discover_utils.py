...
def should_run_discovery(column_names: Set[str], md_map: Dict[Tuple, Dict]) -> bool:
    ...
    """
    internal_logger.debug("should_run_discovery with (%s)...", column_names)
    ...
    for column_name in column_names:
        ...
        if not md_properties:
            internal_logger.debug("Will run discovery because `%s` not in stream metadata", column_name)
            return True

        if md_properties["selected-by-default"] and is_supported_column_type(md_properties["datatype"]):
            internal_logger.debug(
                "Will run discovery because `%s` is selected by default and of supported type", column_name
            )
            return True
...
def schema_for_column(column):  # pylint: disable=too-many-branches
    ...
    if data_type in BOOL_TYPES or column_type.startswith("tinyint(1)"):
        ...
        result = Schema(None, inclusion="unsupported", description=f"Unsupported column type {column_type}")
    return result
...
def create_column_metadata(cols: List[Column]):
    ...
    for col in cols:
        ...
        mdata = metadata.write(
            mdata, ("properties", col.column_name), "selected-by-default", schema.inclusion != "unsupported"
        )

        mdata = metadata.write(mdata, ("properties", col.column_name), "sql-datatype", col.column_type.lower())

        mdata = metadata.write(mdata, ("properties", col.column_name), "datatype", col.data_type.lower())
...
def resolve_catalog(discovered_catalog, streams_to_sync):
    ...
    for catalog_entry in streams_to_sync:
        ...
        if not discovered_table:
            user_logger.warning(
                "Database %s table %s was selected but does not exist", database_name, catalog_entry.table
            )
            continue
        ...
        result.streams.append(
            CatalogEntry(
                ...
                schema=Schema(
                    type="object", properties={col: discovered_table.schema.properties[col] for col in columns}
                ),
            )
        )
...
def desired_columns(selected, table_schema) -> Set:
    ...
    if selected_but_unsupported:
        user_logger.warning("Columns %s were selected but are not supported. Skipping them.", selected_but_unsupported)

    selected_but_nonexistent = selected.difference(all_columns)
    if selected_but_nonexistent:
        user_logger.warning("Columns %s were selected but do not exist.", selected_but_nonexistent)

    not_selected_but_automatic = automatic.difference(selected)
    if not_selected_but_automatic:
        user_logger.warning(
            "Columns %s are primary keys but were not selected. Adding them.", not_selected_but_automatic
        )
